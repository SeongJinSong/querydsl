...
public class QuerydslBasicTest {
    ...
    //스프링이 제공해주는 em 자체가 멀티쓰레드환경에서 동시성문제없이 동작하도록 되어있다.
    ...
    public void startQuerydsl(){
        /**
         * 같은 테이블을 self 조인해야할때만 선언해서 사용하자!
    ...
    public void aggregation(){
         ...
         * Tuple
         * - Querydsl이 제공하는 자료구조
         * - 데이터 타입이 여러개일때 튜블이 쓰면 편하다.
         * - 하지만 실무에서는 튜블은 잘 쓰지 않고 dto를 사용한다.
         */
    ...
    /**
     * 연관관계가 없는 엔티티 외부 조인 (on 조인)
     * - 연관관계가 없는 조인은 join()안에 1개만 넣는다.
     *
     * 예) 회원의 이름이 팀 이름과 같은 대상 외부조인
     */
    @Test
    ...
    /**
     * case 문
     *   - 일반적으로 이런 case문이 쓰는 경우는 애플리케이션 로직 혹은 프리젠테이션 로직에서 하는게 좋다.
     */
    ...
    /**
     * Tuple은 repository 계층에서까지만 사용해라
     */
    @Test
    ...
    public void findDtoByJPQL(){
        List<MemberDto> result = em.createQuery("select new study.querydsl.dto" +
    ...
    /**
     * field를 직접 사용함
     *  - private 이지만 라이브러리에서 리플렉션 기술등을 활용하여 해결
     */
    @Test
     ...
     * 별칭이 다를 때
     * - 필드에 매핑할 별칭이 있는경우 as
     * - 필드에 매칭할 별칭이 없는경우 ExpressionUtils 활용 -> 보통 서브쿼리인 경우
     */
    @Test
    ...
    /**
     * field와 setter는 이름매칭이 중요해서 as나 ExpressionUtils 필요하지만
     * 생성자의 경우 타입만 맞으면 된다.
     */
...
//        Integer ageParam = 10;
    ...
    /**
     * 벌크 연산
     * - 영속성 컨텍스트에 엔티티에 올라가 있다.
     * - 벌크연산은 영속성 컨텍스트를 무시하고 DB에서 바로 실행한다.
     * - 영속성 컨텍스트를 flush, clear 해주어야 한다! -> @Modifying
     */
    @Test
    public void bulkUpdate(){
        ...
        long count = queryFactory
                ...
                .execute();
        /**
         * DB에서 조회해도 영속성 컨텍스트에 이미 키가 있다면
         * 영속성 컨텍스트에 있는 값이 우선순위를 가진다. - repeatable read
         */
        List<Member> result = queryFactory
                .selectFrom(member)
                .fetch();
